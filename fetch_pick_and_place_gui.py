...
base = _load_module("workbench_base.py", "fetch_pick_and_place_workbench_base")
logic = _load_module("fetch_pick_and_place_logic.py", "fetch_pick_and_place_logic")


class FetchPickAndPlaceUI(base.WorkbenchUI):
    def __init__(self) -> None:
        super().__init__()
        self.title("Stable-Baselines3 Workbench - Fetch Pick And Place")
        self.geometry("1280x780")
        self.minsize(1080, 660)
        self.trainer = logic.FetchPickAndPlaceTrainer()
    ...
    def build_workbench_layout(self, build_controls, build_actions) -> None:
        ...
        ttk.Label(header, text="FetchPickAndPlace-v4", style="Accent.TLabel").pack(anchor="w", padx=(6, 0), pady=(2, 0))
    ...
    def _open_animation_window(self) -> None:
        ...
        axes.set_axis_off()
        axes.set_title("Fetch Pick And Place - Animation (Vollbild)", fontsize=14, pad=10, color=self.colors["text"])
    ...
    def _sync_animation_window_frame(self) -> None:
        ...
        if self.render_image is None:
            ...
            self.animation_window_axes.set_title("Fetch Pick And Place - Animation (Vollbild)", fontsize=14, pad=10, color=self.colors["text"])
    ...
    def _build_actions(self, parent: ttk.Frame) -> None:
        for text, command, style in (
            ...
            ("Parameter-Sweep", self.show_parameter_sweep_window, "Primary.TButton"),
    ...
    def _sweep_parameter_names(self, method: str) -> list[str]:
        shared = ["n_envs", "learning_rate", "gamma", "batch_size"]
        if method == "PPO":
            return shared + ["n_steps", "n_epochs", "gae_lambda", "clip_range", "ent_coef", "vf_coef", "max_grad_norm"]
        if method == "SAC":
            return shared + ["buffer_size", "learning_starts", "tau", "train_freq", "gradient_steps", "ent_coef"]
        return shared + ["buffer_size", "learning_starts", "tau", "train_freq", "gradient_steps", "policy_delay", "ent_coef"]

    def _sweep_example_values(self, method: str, parameter_name: str) -> str:
        if method == "PPO":
            examples = {
            ...
            examples = {
                ...
                "learning_starts": "1000,10000,30000",
                "tau": "0.003,0.005,0.01",
                "train_freq": "1,4,8",
                "gradient_steps": "1,4,8",
                "ent_coef": "auto,0.01,0.05",
                "n_envs": "1,2,4",
            }
        else:
            examples = {
        ...
        return examples.get(parameter_name, "1,2,3")

    def show_parameter_sweep_window(self) -> None:
        window = tk.Toplevel(self)
        window.title("Parameter-Sweep")
        window.transient(self)
        window.resizable(False, False)
        window.grab_set()
        ...
        ttk.Label(content, text="Lernmethode auswählen:", style="Section.TLabel").pack(anchor="w", pady=(0, 6))
        method_var = tk.StringVar(value=self.selected_method.get())
        method_combobox = ttk.Combobox(content, textvariable=method_var, values=list(logic.SUPPORTED_METHODS), state="readonly", width=28)
        method_combobox.pack(fill=tk.X, pady=(0, 12))

        ttk.Label(content, text="Parameter auswählen:", style="Section.TLabel").pack(anchor="w", pady=(0, 6))
        parameter_var = tk.StringVar()
        parameter_combobox = ttk.Combobox(content, textvariable=parameter_var, state="readonly", width=28)
        parameter_combobox.pack(fill=tk.X, pady=(0, 12))

        ttk.Label(content, text="Werte (kommagetrennt):", style="Section.TLabel").pack(anchor="w")
        value_var = tk.StringVar()
        ttk.Entry(content, textvariable=value_var).pack(fill=tk.X, pady=(4, 12))

        base_info = ttk.Label(content, text="", style="Status.TLabel")
        base_info.pack(anchor="w", pady=(0, 8))

        def update_parameter_choices(*_args) -> None:
            method = method_var.get()
            parameter_names = self._sweep_parameter_names(method)
            parameter_combobox["values"] = parameter_names
            if parameter_names:
                parameter_var.set(parameter_names[0])
                value_var.set(self._sweep_example_values(method, parameter_names[0]))
                base_config = self._configured(method)
                base_info.config(text=f"Beispiel für {method}: {getattr(base_config, parameter_names[0])}")

        def update_example(*_args) -> None:
            method = method_var.get()
            if parameter_var.get():
                value_var.set(self._sweep_example_values(method, parameter_var.get()))

        def start_selected_sweep() -> None:
            method = method_var.get()
            parameter_name = parameter_var.get()
            selected_values = value_var.get()
            self.start_parameter_sweep(window, method, parameter_name, selected_values)

        method_var.trace_add("write", update_parameter_choices)
        parameter_var.trace_add("write", update_example)
        update_parameter_choices()

        ttk.Button(content, text="Sweep starten", command=start_selected_sweep, style="Primary.TButton").pack(anchor="e")

    def start_parameter_sweep(self, window: tk.Toplevel, method: str, parameter_name: str, values_text: str) -> None:
        if self.training_active:
            self.status_var.set("Training läuft bereits.")
            return
        try:
            ...
            for item in values_text.split(","):
                ...
                if not raw:
                    continue
                if isinstance(getattr(base_config, parameter_name), int):
                    values.append(int(float(raw)))
                elif isinstance(getattr(base_config, parameter_name), float):
                    values.append(float(raw))
                else:
                    values.append(raw)
            if len(values) < 2:
                raise ValueError("Bitte mindestens zwei Werte für den Parameter angeben.")
        except Exception as error:
            self.show_error(error)
            return
        configs = []
        for value in values:
            config = logic.OnPolicyConfig(**{**vars(base_config), parameter_name: value})
            config.sweep_label = f"{method} {parameter_name}={value}"
            configs.append(config)
        self.reward_history = {config.sweep_label: [] for config in configs}
        self.draw_sweep_plot({config.sweep_label: [] for config in configs}, axes=self.axes)
        self.canvas.draw_idle()
        ...
        self.running_workers = len(configs)
        self.progress_var.set(0)
        self.status_var.set(f"Sweep läuft: {method} / {parameter_name}")
        self.selected_method.set(method)
        self.axes.set_title(f"{method} - Parametervergleich: {parameter_name}", color=self.colors["text"])
        for config in configs:
    ...
    def start_training(self, method: str) -> None:
        ...
        if hasattr(self, "animation_button"):
            self.animation_button.configure(text="Animation AUS")
        self.animation_state_var.set("Live-Animation eingeschaltet")
        self.training_active = True
        self.stop_event = threading.Event()
        self.pause_event = threading.Event()
    ...
    def _training_worker(self, config: logic.OnPolicyConfig) -> None:
        def emit(event: dict[str, Any]) -> None:
            ...
            if event_method is None:
                event["method"] = config.sweep_label or config.method
            elif event_method == config.method and config.sweep_label is not None:
                event["method"] = config.sweep_label
            self.events.put(event)
        try:
            result = self.trainer.train(config, self.stop_event, emit, render_during_training=self.training_animation_visible, frame_capture_steps=self.training_frame_capture_steps, pause_event=self.pause_event)
            self.events.put({"type": "finished", "method": config.sweep_label or config.method, "result": result})
        except Exception as error:
            self.events.put({"type": "error", "error": error})
    ...
    def _live_demo_worker(self, model: Any, config: logic.OnPolicyConfig) -> None:
        environment = logic.make_fetch_env(render_mode="rgb_array")
        try:
    ...
    def show_training_frame(self, frame, method: str | None = None) -> None:
        if not hasattr(self, "render_axes") or self.render_axes is None:
            return
        if not hasattr(self, "render_canvas") or self.render_canvas is None:
            return
    ...
    def _flush_animation_frame(self) -> None:
        ...
        if self.render_image is None or self.render_image.axes is None:
            ...
            self.render_axes.set_axis_off()
            self.render_axes.set_title("Fetch Live", fontsize=13, pad=10, color="#e8f3ff")
        else:
